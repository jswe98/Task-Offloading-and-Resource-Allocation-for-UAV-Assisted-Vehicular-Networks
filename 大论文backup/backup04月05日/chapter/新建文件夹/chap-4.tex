...
& \frac{D_R}{\nu_m} \geq t_{m}^{mec}+t_{wired} , \forall m                           \tag{\ref{E4-13}{-4}}      \label{E4-13-4}\\  %这个是时隙的约束
...
\end{align}
类似于问题 \eqref{E4-36}，问题 \eqref{E4-29}的非凸问题部分转化成了凸问题，并且是容易求解的形式，可以使用CVX工具箱进行迭代求解。%再加一行行不行再加一行行不行再加一行行不行再加一行行不行再加一行行不行再加一行行不行再加一行行不行行不行再加一行
...
下图展示了三种方案在不同参数$\theta$下的的对比。在图 \ref{不同的参数在不同的方案下的能效}中，探究了OPTS、FHCS和FTS三种无人机通信方案的能效如何随参数$\theta$的变化而变化。
\begin{figure}[H]
\centering
