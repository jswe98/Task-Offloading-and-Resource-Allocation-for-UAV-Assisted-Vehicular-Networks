...
\subsection{通信模型与能耗模型}\label{section4-2-1}
...
& q_U^{n\mathrm{\ }+1}-q_U^{n\mathrm{\ }}\le tV_{max}, \forall m         \tag{\ref{E4-3end}{-2}}      \label{E4-3end2}\\  %无人机飞行轨迹
& x_m\left[t\right] + y_m\left[t\right]=1, \forall m                     \tag{\ref{E4-3end}{-3}}      \label{E4-3end3}\\  %时隙分配加起来是一
& 0\le p_m\left[t\right]\le p_{max}, \forall m                           \tag{\ref{E4-3end}{-4}}      \label{E4-3end4}    %功率阈值
\end{align}
...
在求解车辆发射功率的过程中，需要每个时隙都要进行功率分配与无人机轨迹规划并进行多次迭代，
...
对于凸问题\eqref{E4-5end}，我们可以构建拉格朗日函数并运用拉格朗日对偶法进行求解：
\begin{align}\label{E27}
&L\left(\mathbf{p},\lambda\right)\\
&=\sum_{t=1}^T \sum_{m=1}^M {B}_{0}x_m\left[t\right]\log_2{\left(1+\gamma_{m,R}\left[t\right]\right)}+ \notag
\sum_{t=1}^T \sum_{m=1}^M {B}_{0}y_m\left[t\right]\log_2{\left(1+\gamma_{m,U}\left[t\right]\right)}                     \\ \notag
&-\chi \sum_{t=1}^T \sum_{m=1}^M{(1-\theta){E}_{m,R}\left( \mathbf{P} \right)+\theta{E}_{m,R}\left( \mathbf{P} \right)} \\ \notag
&-\sum_{t=1}^T\sum_{m=1}^M\lambda_{m,t}\Big[p_m\left[t\right]x_m\left[t\right]\ln \left(1-a \varepsilon_3\right)+(\gamma_{th}        \\ \notag
&-y_m\left[t\right]\gamma_{m,U}) a \sigma^2-a p_m\left[t\right]x_m\left[t\right]\hat{h}_{m}\left[t\right]\Big]   \notag,
\end{align}
...
{(1-\theta){{E}_{\text{t}}}^{R,\left\{ l \right\}}+\theta{{E}_{\text{t}}}^{U,\left\{ l \right\}}}}         \label{E4-6end}\\
\text { s.t. }
& \eqref{E4-3end2}                                                       \tag{\ref{E4-6end}{-1}}           \label{E4-6end1}
\end{align}
我们注意到问题\eqref{E4-6end}目标函数的分子部分是非凹问题，拟采用连续凸逼近方法对目标函数进行近似。在${{q}^{\left\{ l \right\}}}\left[t\right]$ 
的局部点处，对式\eqref{E4-6end}的分子部分的对数形式进行一阶泰勒展开，过程如下：  
