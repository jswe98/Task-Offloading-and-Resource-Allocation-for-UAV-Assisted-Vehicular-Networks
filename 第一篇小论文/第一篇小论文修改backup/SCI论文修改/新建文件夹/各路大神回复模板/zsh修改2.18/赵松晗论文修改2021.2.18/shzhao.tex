...
\author{Zhixin Liu, Songhan Zhao, Yazhou Yuan, Yi Yang, Xinping Guan

\thanks{Zhixin Liu, Songhan Zhao, Yazhou Yuan and Yi Yang
are with the School of Electrical Engineering, Yanshan
University, Qinhuangdao 066004, China. Emails: lzxauto@ysu.edu.cn,
zsh951227233@163.com, yzyuan@ysu.edu.cn, yyi@ysu.edu.cn}
...
\end{eqnarray}
where {\color{blue}$P_p$ denotes the power of PT in phase two, and $h_{pp}$ is the channel power gain between PT and PR.} $h_{ip}$ is the channel power gain between SU$_i$ and PR. $N$ is the number of SUs, and $P_{ci}$ is the power of SU$_i$ in relaying link. $N_0$ is the additive white Gaussian noise (AWGN) power, which is the same constant for all users. The achievable throughput of the PU is expressed as \cite{Kalamkar2016}
...
Note that $F_{i}^{(k)}$ is the form of accumulation, which can completely reflect the behavior of SU$_i$ in the whole iterative process. \par
...
\end{eqnarray}
where the first term represents the profit that SU$_i$ obtains from the cooperation. The second term, i.e., $aF_{i,(n)}P_it_a$ can be interpreted as the cost that SU$_i$ uses harvested energy to transmit its own data in phase four, where $a$ is the equilibrium coefficient. It is worth noting that the selfish behavior of SU$_i$ will lead to the increasing of the cost. Therefore, a feedback mechanism is established to improve the fairness among the SUs. \\
...
\end{eqnarray}
{\color{blue} where $b$ is the equilibrium coefficient.} And the first term denotes the throughput obtained by the cooperation protocol. The second term represents the energy consumed in the rewarded period.\\
\indent The optimal power allocation policy in relaying link ${P_{ci}^{\ast}}$, the optimal time allocation policy $ t_a^{\ast}$ and $t_c^{\ast}$, can be obtained by solving the optimization problem \eqref{11}, where $\bm {P_{cs}}=\left[P_{c1},\cdots, P_{cN}\right]$. $P_i^{\ast}$ denotes the best response of SU$_i$, and the constraint (11d) indicates that the PU anticipates the decisions of the SUs which act as followers, and make the optimal strategy based on the best response of the followers. Constraint (11c), (11e), (11f), and(11g) denote the range constraints of variables.
...
In this section, an iterative algorithm is proposed for solving (\ref{9}) and (\ref{11}) based on Stackelberg game. {\color{blue} The leader, PU, first adjusts its strategy based on the best response of the followers (i.e., the SUs).} Then each SU$_i$ reacts, i.e., it selects one strategy to maximize its utility.
%--------------------------------------------------------------------------------
...
\end{split}
\end{eqnarray}
 where $a,c$ are the coefficients defined in \eqref{7} and \eqref{8}.
...
\indent Since the Slater's condition is satisfied in problem (\ref{9}), the strong duality holds \cite{Boyd2004}. Therefore, the best response $P_i^{\ast}$ in (\ref{9}) can be obtained via Lagrange dual decomposition.
...
\end{align}
The best response $P_i^{\ast}$ can be obtained by using Karush-Kuhn-Tucker (KKT) conditions \cite{XUY2020}, which are given as
...
Therefore, the signal overhead of Algorithm 1 is $2N$, where $N$ is the number of the SU$_i$. {\color{blue}And the computational complexity is $\mathcal{O}(N)$. For a centralized algorithm in reference  \cite{Venturino2009}, each SU$_i$ needs the status information from the other SUs. The signal overhead is increased to $N^2+N$ and the computational complexity is $\mathcal{O}(N^2)$}. Through the above analyses, our algorithm has lower complexity and less signal overhead compared with the centralized algorithm.
\section{Simulation Results}
In this section, the simulation results are presented to evaluate the performance of the proposed resource allocation algorithm.  Firstly, the convergence of the proposed algorithm is analyzed. And then, we compare the harvested energy with the allocated energy by each user, which can demonstrate the effectiveness of our energy allocation policy. Furthermore, GFRAA is compared with two other algorithms, namely  equal power and optimal time allocation (EPOTA) \cite{Xu2017} and optimal time and power but non-fairness allocation (OTPNF) {\color{blue}\cite{DXu2019}} in different scenarios. To guarantee the comparability of the simulation results, EPOTA allocates power equally in the rewarded period and optimizes the other variables in the same way as GFRAA. OTPNF optimizes time and power by the same method as GFRAA, but does not introduce the fairness evaluation coefficient. {\color{blue}Moreover, three algorithms adopt the same parameter values from Table1.}
...
\centering
%\hspace{1.0cm}
\includegraphics[width=6.5cm]{EHandEAs.eps}
\caption{Energy statuses of SUs with $P_e=30$ dBm, $N=10$ and $\bar{R}_a=0.5$ Mbps.}
\end{figure}
...
\begin{minipage}{7cm}
\centering
\includegraphics[width=6.5cm]{Fairness1.eps}
\end{minipage}
}
\subfigure[The sum-throughput of SUs]{
\begin{minipage}{7cm}
\centering
\includegraphics[width=6.5cm]{Fairness2.eps}
\end{minipage}
}
\subfigure[The throughput of PU]{
\begin{minipage}{7cm}
\centering
\includegraphics[width=6.5cm]{Fairness3.eps}
\end{minipage}
}
\caption{Effect of the number of SUs on (a), (b) and (c) with $P_e=30$ dBm and $\bar{R}_a=0.5$ Mbps.}
\end{figure}
...
\centering
\includegraphics[width=6.5cm]{targetofsu2.eps}
\end{minipage}
}
\subfigure[The throughput of PU]{
\begin{minipage}{6.5cm}
\centering
\includegraphics[width=6.5cm]{targetofsu1.eps}
\end{minipage}
}
\caption{Effect of the target secondary throughput on (a) and (b) with $P_e=30$ dBm and $N=10$.}
\end{figure}
...
\centering
%\hspace{1.0cm}
\includegraphics[width=6.5cm]{PE.eps}
\caption{Effect of PT's transmit power $P_e$ with $N=10$ and $\bar{R}_a=0.5$ Mbps.}
\end{figure}

{\color{blue} Fig. 9 investigates the PU throughput versus the PT's transmit power $P_e$. The PU throughput of three algorithms increase as the $P_e$ increases. The reason is that, as the $P_e$ increases, SUs will harvest higher energy, which leads to the increase of available energy in relaying process. Furthermore, the evaluation coefficient are introduced in GFRAA, which can inhibit the selfish behavior that SUs tend to consume more energy for their own transmissions. Hence, GFRAA can allocate higher energy for relaying, and achieve a better performance of PU throughput compared with EPOTA and OTPNF.}

\begin{figure}[H]
\centering
%\hspace{1.0cm}
\includegraphics[width=6.5cm]{R.eps}
\caption{Effect of SUs’ distribution around SAP with $P_e=30$ dBm, $N=10$ and $\bar{R}_a=0.5$ Mbps.}
\end{figure}
...
\end{split}
\end{eqnarray}
 where $a,c,d$ are the coefficients defined in \eqref{7} and \eqref{8}.\par
\indent We make the partial derivative equal to zero, and we can get:
\begin{eqnarray}\label{32}
\begin{split}
&t_a\frac{Wh_i}{N(N_0+P_ih_i)}=\!at_aF_i^{(k-1)}\!\!-\!\!cdat_a+\\
&~~~~~~~~~~~~~~~~~~~~~~~2cat_a\frac{P_ih_i}{\sum\nolimits_{j=1,j\not= i}^{N}P_{j}^{(k-1)}h_j}\!+\!\mu_it_a,\\
&\Rightarrow \underbrace{\frac{2cah_i^2}{\sum\nolimits_{j=1,j\not= i}^{N}P_{j}^{(k-1)}h_j}}_{A_i}P_i^2\\
&+\!\underbrace{\left[\frac{2acN_0h_i}{\sum\nolimits_{j=1,j\not= i}^{N}P_{j}^{(k-1)}h_j}\!+\!\mu_ih_i\!+\!aF_i^{(k-1)}h_i\!-\!acdh_i\!\right]}_{B_i}\!P_i\\
&+\underbrace{N_0\left(\mu_i+aF_i^{(k-1)}-acd\right)-\frac{W}{N}h_i}_{C_i}=0.
\end{split}
\end{eqnarray}
...
\indent We first introduce a conclusion which is given as follows \cite{Xu2017}.
For a equation with following form,
...
x=\frac{-\mathcal{W}(-e^{-b-1})-1}{\mathcal{W}(-e^{-b-1})},
\end{eqnarray}
where $a$ and $b$ are constants, $\mathcal{W}(\cdot)$ denotes the Lambert W function. \\

Then we take partial derivative of $\textit{L}(t_c,\bm\nu,\gamma)$ with respect to $t_c$, that is
...
\end{split}
\end{eqnarray}
\indent Making the above partial derivative equal to zero, we can derive:
\begin{eqnarray}\label{36}
\begin{split}
&\ln\left(1+\frac{P_ph_{pp}}{N_0}+\sum\nolimits_{i=1}^{N}\frac{E_{ci}h_{ip}}{t_cN_0}\right)\\
&-\frac{\sum\nolimits_{i=1}^{N}\frac{E_{ci}h_{ip}}{t_cN_0}}{1+\frac{P_ph_{pp}}{N_0}+\sum\nolimits_{i=1}^{N}\frac{E_{ci}h_{ip}}{t_cN_0}}=\frac{2}{W}(\eta P_e\sum\limits_{i=1}^{N}\nu_ih_{pi}+\gamma).
\end{split}
\end{eqnarray}
\indent It can be seen that when we make $a=1+\frac{P_ph_{pp}}{N_0}$, $b=\frac{2}{W}(\eta P_e\sum\limits_{i=1}^{N}\nu_ih_{pi}+\gamma)$, and $x=\sum\nolimits_{i=1}^{N}\frac{E_{ci}h_{ip}}{t_cN_0}$, Eq. \eqref{36} is converted into the identical form of Eq. \eqref{33}. Therefore, by employing the same method in Eq. \eqref{34} and taking some mathematical manipulations, the optimal $t_c^\ast$ can be obtained, which is given by Eq. \eqref{29}.
