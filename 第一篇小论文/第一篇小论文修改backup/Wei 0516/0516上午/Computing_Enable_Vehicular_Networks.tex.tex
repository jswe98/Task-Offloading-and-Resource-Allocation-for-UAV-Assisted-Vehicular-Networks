...
{We denote the number of CPU cycles required to process 1-bit of input data at vehicle $i$ as $c_0$.} \cite{Zhang2017}, which is indivisible and cannot be broken down into smaller components \cite{Saleem2021}. $c_0$ can be obtained through carefully profiling of the task execution \cite{Yang2015}. We consider that each vehicle $v\in \mathcal{V}$ has a different computation task at a time, denoted as $T_i$, is defined by a tuple consisting of two parameters, $\langle d_{i,up}, c_{i,e}\rangle$, in which $c_{i,e}$ [cycles] specifies the workload \cite{Tran2019}. Hence, the computation cost to accomplish the task, $c_{i,e}$ can be obtained through $c_{0}*d_{i,up}$ \cite{Shuang2021}. Each task is offloaded to the MEC server and then transmit to the cloud servers. By offloading the computation task to the MEC servers, the vehicles have more computing resources. However, additional time is likely to be consumed for transmitting the task input in the uplink direction.
...
\end{eqnarray}
where $t_{max}$ is the maximum time of the task completion tolerable threshold. If a task is completed within $ t_{max}$, the vehicle has a higher utility. Otherwise, it produces the corresponding loss. Therefore, we define the offloading utility of vehicle $i$ as $\frac{U_{i,exe}}{t_{i,up}}$, which is the offloading utility function per unit of time.
...
\end{IEEEproof}
Therefore, {transform the deterministic optimization problem of robust power allocation given by equation \eqref{E26}, we can reformulate the objective function, outage probability constraints, and delay constraints as follows:}
...
\end{equation}
{where $K_\lambda$ and $K_\mu$ represent the step size for the Lagrangian multipliers, $K_\lambda\geq0$ and $K_\mu\geq0$. The variable $t$ denotes the iteration index, and $\left[x\right]^+=\max{\left[0,x\right]}$ represents the positive part of the variable $x$.}
...
\end{align}
	...
	\label{eq}
\end{subequations}
Notice that the constraints in \eqref{E32b} and \eqref{E32c} are convex. By using the second-order derivatives of $f_i$, the Lagrangian function is constructed to determine the optimal powers. Hence, the Lagrangian function of \eqref{E32} is formulated as,
