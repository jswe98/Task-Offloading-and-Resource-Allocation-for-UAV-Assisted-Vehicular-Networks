...
以满足爆炸式增长的计算卸载需求[40–42]。然而，云计算中心往往远离主干道，导致
云计算延迟较长并造成网络拥堵与隐私泄露等问题[43–45]。在高动态车联网中，车辆
...
The system model is depicted in Fig. In reference to Fig. [systemuav2], we consider an integrated celestial and terrestrial network in which vehicles are travelling on a bidirectional highway. UAVs take off from the vicinity of the base station and cache the resources provided by the base station for download by the vehicles on the road. This is because the highway is bidirectional. The base station is situated at the origin of the coordinates at a height of $h_0$. The $D_R$ represents the radius length of the coverage of the roadside cell. In order to consider a more realistic road scenario, we model the vehicle motion as a constant velocity motion model, as described in reference \cite{Fiore2008TheNS}. We define the positive direction as the right and the negative direction as the left. The lane index $L=1$ represents a vehicle travelling to the right, while $L=-1$ represents a vehicle travelling to the left. Due to the fixed location of the base station, the vehicle's direction will inevitably diverge from the base station over time. This will inevitably affect its access to information through the base station. At this juncture, the UAV flew towards the right side of the base station, which in turn helped the vehicle to obtain the necessary information. In order to ascertain whether the vehicle on the road requires information from the drone or the base station, we considered the channel state information predicted by the first-order Markov process from the vehicle to the base station and the channel state information obtained by the vehicle and the aerial base station drone line-of-sight link to the vehicle. The vehicle will select the party with the larger signal-to-noise ratio to request resources. This is expressed as $x_m\left[t\right]=1$. Conversely, the vehicle will select the UAV for communication if $x_m\left[t\right]=0$. Similarly, the vehicle will select the base station for communication if $x_m\left[t\right]=0$.

In the time slot $t$, the horizontal coordinates of the drone are $q_U\left[t\right]=\left\{\pi_u\left[t\right],\varpi_u\left[t\right]\right\}$.
The UAV is flying unobstructed at a height of $H$ from the road surface with a maximum speed of $V_{max}$ and the initial horizontal position of the vehicle $M$ is $q_m\left[0\right]=\left\{\pi_0,\varpi_0\right\}$.
...
The horizontal position changes to $\pi_m\left[t\right]=\pi_0+l\nu_m t$, and the horizontal position of vehicle $M$ is $q_m=\left\{\pi_m\left[n\right],\varpi_0\right\}$.
Based on the position information we can get the distance information at the $t$th moment
The distance between vehicle $M$ and the roadside unit at $t$ time slot is ${{d}_{m,R}}\left[ t \right]=\left\| {{q}_{M}}\left[ t \right]-{{q}_{R}} \right\|=\sqrt{{{\pi}_{m}}{{\left[ t \right]}^{2}}+{{\varpi}_{0}}^{2}+{{H}^{2}}}$
,The distance of vehicle $M$ from the UAV at $t$ time slot is $ {{d}_{m,U}}\left[ t \right]=\left\| {{q}_{M}}\left[ t \right]-{{q}_{U}}\left[ t \right] \right\|=\sqrt{{{\left( {{\pi}_{m}}\left[ t \right]-{{\pi}_{u}}\left[ t \right] \right)}^{2}}+{{\left( {{\varpi}_{m}}\left[ t \right]-{{\varpi}_{u}}\left[ t \right] \right)}^{2}}+{{H}^{2}}}\ $.
...
Given the high velocity of vehicular movement, a first-order Markov process analogous to that described in the preceding chapter is employed for the vehicle-to-infrastructure (V2I) communication between the vehicle and the roadside unit. In this process, the channel state information at the $t$th moment is predicted from the state prediction of the previous moment.
i.e.
...
\end{equation}
The quantity of data transmitted by the vehicle to the terrestrial base station can be quantified as,
...
\end{equation}
The variable $B_0$ denotes the bandwidth, and the value $x_m[t] = 1$ indicates that the vehicle has chosen to transmit data to the terrestrial base station at the current moment, and $x_m[t] = 0$ indicates that the vehicle has chosen to refrain from transmitting data to the terrestrial base station at the current moment.
...
In order to establish a line-of-sight link between the vehicle and the UAV, which is necessary for communication without obstacle occlusion in the middle, an airborne RF link model is constructed. The channel gain from the $m$ vehicle to the UAV at the $t$th time slot is,
\begin{equation} \label{E4-7}
h_{m,U}\left[t\right]=\frac{\varsigma_0}{{d_{m,U}\left[t\right]}^2}
\end{equation}
Here the symbol $\varsigma_0$ represents the power gain per unit distance of one metre. With the aforementioned information in mind, the transmission rate from the unmanned aerial vehicle (UAV) to the vehicle can be calculated as:
...
\end{equation}
the signal-to-noise ratio of the vehicle-to-UAV communication is denoted by the symbol. The amount of data transmitted by the vehicle to the UAV airborne base station can be expressed as,
...
The \eqref{E4-13-1} in the aforementioned equation represents the interruption probability constraint, which is designed to guarantee the quality of service by ensuring vehicle communication. The \eqref{E4-13-2} represents the constraint on the maximum and minimum transmit power of the vehicle, The equation \eqref{E4-13-3} demonstrates that the task of each vehicle at any given moment in time can only be offloaded to either the UAV or the terrestrial base station. The threshold of the signal-to-noise ratio, denoted by $\gamma_{th}$, is a fixed value, \eqref{E4-13-4} represents the necessity to dispatch the vehicle before it leaves the roadside cell coverage area. Prior to the server completing the computation of completion data and the allocation of time slot resources, this constraint can be employed. The time point $t_{wired}$ is defined as the time at which the roadside unit transmits data to the edge server for a fixed duration. The flight trajectory of the UAV is constrained by the inequality given in Equation \ref{E4-13-5}, which states that the actual distance flown by the UAV per second must be less than or equal to the maximum flyable distance of the UAV.
\begin{comment}
...
This is a fractional programming problem. We can use a Dinkelbach method to transform it into an easily solvable reduced-form programming problem. Equation \eqref{E4-36} is equivalent to a linear programming problem that can be solved iteratively using the Convex Optimisation Toolbox (CVX).
\subsection{Overall Algorithm Design}
The original problem is divided into three subproblems, which are solved separately in the above subsections. They are then solved alternatively using an alternating iteration method, shown in Algorithm~\ref{alg:algorithm1}.
\begin{algorithm}[ht]
...
Firstly, we show how to optimise the UAV trajectory to maximise the system's energy efficiency for different time slots. The black arrow represents the vehicle travelling to the right. We stipulate that the UAV flies from a fixed starting point to a fixed end point. When there's enough time, the UAV will stay close to the far vehicle to help it. Figure \ref{UAVspeed} shows the drone's speed at each time slot. When the vehicle needs help, the drone slows down or hovers to match the vehicle's speed.
\begin{figure}[H]
\centering
...
After testing the optimised algorithm, we tested the scheme with different parameters. Fig. \ref{UAV Trajectory Optimisation} shows that different $\theta$ affect energy efficiency at different times. $\theta$ is a coefficient that compares energy costs at the drone and roadside units. When $\theta$ changes, the system utility also changes. Higher parameters mean more focus on the UAV's flight energy consumption. The overall system energy efficiency decreases as the UAV flight time $T$ increases. Longer time allows the UAV to traverse more vehicles to obtain better channels and save energy. Lower $\theta$ leads to higher energy efficiency. This is because when we consider the UAV's energy consumption to be important, vehicles increase their power to transmit more data, thus increasing energy efficiency.
\begin{figure}[H]
\centering
...
Fig. \ref{DifferentSchemes} compares the energy efficiency of three UAV communication schemes, OPTS, FPHS and FTS, under different flight cycles T. As the UAV's flight time increases, each scheme's energy efficiency gradually stabilises. This is because the UAV has more time to transmit data at the right power. Further observation shows that the two schemes, OPTS and FPHS, are suitable for different scenarios. OPTS is better for situations where fast response is required. If the flight cycle is longer, FPHS is simpler to implement than OPTS. Therefore, in environments with less time requirements, FPHS may be a more economical solution. However, FPHS is less energy efficient. The UAV hovers at a fixed position, which affects the efficiency of data transmission. As the flight cycle increases, the energy efficiency gap between OPTS and FPHS narrows. When the flight cycle is short, the OPTS has less time to send data to the UAV, so it uses less energy. As the flight cycle increases, the OPTS has more time to send data, so it uses more energy.
\begin{figure}[H]
\centering
...
Fig. \ref{DifferentSchemesANDdifftheta} shows how the energy efficiency of three UAV communication scenarios, OPTS, FHCS and FTS, varies with the parameter $\theta$. The parameter $\theta$ reflects the weighting of the relative energy cost between the UAV's flight energy consumption and its communication energy consumption. When $\theta$ is large, the UAV's flight energy consumption is the main factor in overall energy consumption. The figure shows that as $\theta$ changes, the energy efficiency of all three scenarios decreases and reaches a low point at $\theta=0.8$. It then increases slightly.
When $\theta$ is small, the UAV's flight energy consumption is low, so it can transmit data more. This improves energy efficiency. When $\theta$ is large, the UAV's flight energy consumption is high, so it can't transmit data, which affects energy efficiency. The parameter $\theta$ affects the energy efficiency of the three UAV communication schemes by influencing how much energy is used for flight and communication. In practice, we need to choose the right $\theta$ value for each situation to make UAV communication more efficient.
\begin{figure}[H]
\centering
