...
\usepackage{fontspec}
%\setmainfont{Latin Modern Roman} % 设置默认字体为Latin Modern Roman，或者你喜欢的其他字体
\newfontfamily\timesfont{Times New Roman} % 定义一个新的字体族为Times New Roman
%\newfontfamily\timetimefont{Time} % 定义一个新的字体族为Times New Roman
...
\newcommand{\mykeywords}{车联网；无人机通信；吞吐量最大化；中断概率；边缘计算；轨迹优化；任务卸载 } %
\newcommand{\myenkeywords}{Internet of Vehicles; UAV Communication; Throughput Maximisation; Outage Probability; Edge Computing; Trajectory Optimisation; Task Offloading} %
...
\title{高动态异构车联网络资源联合鲁棒优化分配研究}% 论文题目  高动态车联异构网络通信资源联合鲁棒优化分配的研究
\school{燕山大学}                                 % 院校名称
\author{魏建帅}  %魏建帅
\advisornamecn{ 刘志新}     %刘志新                      % 导师姓名，注意：姓在前，名在后，若是“副教授”，加上“副”字！！！
\enauthor{Wei Jianshuai}        %Wei Jianshuai    xxx               % 作者英文姓名，注意：姓在前，名在后！！！
\enadvisorname{ Liu Zhixin} %Liu Zhixin           xxx
\makeatletter
\hypersetup{
    pdftitle={\CAST@value@title},
    pdfauthor={\CAST@value@author},
    pdfsubject={ 硕士学位论文},  %\CAST@value@school
    pdfkeywords={\mykeywords\myenkeywords},
...
%%===========================%
%% 文档参数配置结束
\newcommand{\addabstracttotoc}[1]{%
  \addtocontents{toc}{\protect\contentsline{chapter}{\protect\fontspec{Times New Roman}\protect #1}{}{}}%
}
\usepackage{titlesec}
\usepackage{titletoc}%%%%%mzhy55 解决目录中章标题的黑体问题
\titlecontents{chapter}[0pt]{\heiti\zihao{-4}\vspace{0mm}}
...
\defenddate{2024年5月}                            % 答辩日期，注意：要依实际答辩时间修改！！！
%\school{燕山大学}                                 % 院校名称

% 英文封面内容
\enstatement{Control Science and Engineering}     % 学科或领域
\enmajor{Master of Engineering}%{Science}      % 学科或领域
\entitle{Joint Robust Optimal Allocation and Communication Resources Under High Dynamic Vehicular Heterogeneous Networks}                      % 英文题目，所有字母大写！！！
%\enauthor{ Wei Jianshuai}        %Wei Jianshuai                   % 作者英文姓名，注意：姓在前，名在后！！！  Task Offloading and Resource Allocation for UAV-Assisted Vehicular Networks
