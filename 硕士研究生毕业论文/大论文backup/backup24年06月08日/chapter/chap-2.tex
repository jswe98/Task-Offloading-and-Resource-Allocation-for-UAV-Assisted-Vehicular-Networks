...
驾驶体验和事故处理。然而，由于车辆密度和用户需求的快速增加，单小区网络的频谱效率变得较低 \supercite{TFL}。因此，异构车联网络的部署已成为一种趋势 \supercite{ACAR}。
...
面临两大挑战。首先，当使用信道复用模式来提高频谱效率时，多用户干扰是一个棘手的问题。有效和鲁棒的通信在很大程度上受到多用户干扰的影响，特别是在不确定的信道环境中，因此实现有效的干扰管理是一个重大挑战\supercite{CCO}。 其次，空地异构车辆网络(AGHVN)是分层的，其中蜂窝用户CUE和车辆用户VUE分别充当领导者和追随者。然而，CUE和VUE是不同的利益相关者，他们为自己的利益而竞争，平衡各方利益是一项挑战，使用博弈论的决策方法可以有效的构建CUE和VUE之间复杂利
...
本章提出了一种基于博弈论的无人机辅助异构车联网络的鲁棒功率控制方法，以车辆用户与地面宏用户之间的博弈关系为核心，制定了实时功率分配和定价策
略以实现用户利益的最大化。
...
R_{n}=W\log(1+\bar{\gamma}_{n}(p_n)),\quad  n\in \mathcal{S}_l
\end{eqnarray}
\subsection{博弈问题描述}\label{section2-2-2}
在空地一体化异构的车载网络(AGHVN)中，频谱所有者 CUE 可以对干扰进行定价，并将 VUE 的收费作为其利润。在 V2U 小区中，VUE 的效用是传输速率与购买干扰成本之间的差额。
考虑到 CUE 和 VUE 都是自私自利的，它们都愿意为自己的利益而竞争。因此，数学框架符合Stackelberg博弈模型，其中 CUE 和 VUE 分别是领导者和追随者。此外，还考虑了
 CUE 的通信约束。第$n$ 个V2U 单元的下子博弈可表述为：
\begin{comment}
...
此外，通过寻找子博弈的纳什均衡(Nash Equilibrium, NE)，可以得到所提出的Stackelberg博弈的博弈均衡(Game Equilibrium, GE)。关于NE和GE在文献 \cite{RAI}中有如下的详细描述：

设 $c_i^*$ 是上层子博弈优化问题 \eqref{E2-12} 的解，$p_i^*$ 是下层子博弈优化问题 \eqref{E2-11} 的解。那么，对于任意 $(c_i,p_i)$，如果满足以下条件，点 $(c_i^*,p_i^*)$就是所提议的Stackelberg博弈的 GE：
...
一般而言，为了获取Stackelberg博弈的通用均衡，首先需要确定其子博弈的完美纳什均衡。在Stackelberg博弈框架下，车辆用户和宏用户进行非合作博弈，竞争各自的利益。在下层网络中，每个用户均致力于通过优化其功率策
...
\frac{-\gamma_{th} I_{n}}{\ln(1-\varepsilon)}-p_n g_{n,n}\leq0,\quad\forall n\in\mathcal{S}_l%\mathcal{N}
\end{equation}
\subsection{下层子问题的求解}\label{section2-3-2}
...
存在性： 类似于文献 \cite{GT} 的描述，在Stackelberg子博弈 \eqref{E2-12}中存在一个NE，其条件为：
...
\end{equation}
由于 $U_{\textrm{n}}(p_n)$ 相对于 $p_n$ 的二阶导数总是小于 0，所以 $U_{\textrm{n}}(p_n)$ 在 $p_n$ 中是凹的。因此，在Stackelberg子博弈 \ref{E2-12}中存在一个NE 。
\end{proof}\par

唯一性： 当$g_{n,n}$$>$$\sum\limits_{k=0,k\neq n}^N g_{k,n}$时，在所提的Stackelberg子博弈中，NE是唯一的。
...
$\delta p_{n}(t+1)$ 在迭代一段时间后可以趋近于零，而 $p_{n}(t+1)$ 可以趋近于唯一的最优点 $p_{n}^*$。因此，在Stackelberg子博弈中，NE 是唯一的。
\end{proof}
\subsection{上层子问题的求解}\label{section2-3-3}
...
\subsection{Stackelberg博弈的迭代算法}\label{section2-4-1}
本节提出了一种基于Stackelberg博弈的鲁棒资源分配算法来解决优化问题 \eqref{E2-13}和 \eqref{E2-14}。该算法如下所示：
\begin{center}
\begin{tabular*}{\hsize}{@{\extracolsep{\fill}}l l l l}
    \toprule
    \zihao{-5}算法2-1基于Stackelberg博弈的鲁棒资源分配算法                                                             \\
    \midrule
   \zihao{-5} Step1： 开始。                                                                                            \\
...
本节将进行数值模拟，以评估基于鲁棒的Stackelberg博弈的资源分配算法的性能。在半径为500m的 BS 覆盖范围内，模拟了一个包含 1 个 CUE 和 9 个 V2U 集群的简化车辆通信模型，
...
基于鲁棒Stackelberg博弈的资源分配算法的收敛性能如图 \ref{功率收敛性能} 和图 \ref{价格收敛性能} 所示。
\begin{figure}[H]
\centering
...
\end{figure}
\end{comment}
如图 \ref{中断概率对此} 所示，当中断概率的阈值 $\varepsilon$ 在 0.1 到 0.6 之间变化时，用户的实际中断概率总是小于给定的阈值。结果证实，本章方法不仅实现了有效的干扰管理，还保证了传输的鲁棒性。
...
由于道路上车辆共享的带宽有限，当车辆数量增加时，车辆的发射功率会减小以减小对宏用户的干扰，未使用博弈论的方式由于难以找到每个车辆用户的发射功率平衡，所以优化的功率结果相对保守，未能使带宽资源得到最大的利用。
...
概率形式非凸且难以处理，因此在凸优化过程中采用了指数积分法。根据仿真结果，功率值和价格在几步内收敛到最优值。还可以得出结论，Stackelberg博弈优化方
