function [GVR,GVU,distanceVR,distanceVU,CARposition,L] = CARinfo(H,T,M,det,roadlength,roadwidth,UAVcvx,programmatic,thetatheta,BCDl,chedao)
...
vpoy=random_numbery+roadwidth/2*ll(m);
if vpoy<0.5*roadwidth&&chedao==1
    vpoy=0.5*roadwidth+vpoy;
else
    1;
end
     Vposition(m,:,1)=[Vposition_x;vpoy; 0];%第二个是车辆的上下位置
     for t=2:T
...
if chedao == 1
save('单向车道temp.mat', 'Vposition'); 
else
save('双向车道temp.mat', 'Vposition'); 
end      
end
% programmatic = 1
% BCDl = 1
% cardatatemp=Vposition;

if chedao == 1 
Varin2=load('单向车道信息.mat');
Vposition = Varin2.Vposition;  %调用了之前的数据S
else
Varin2=load('双向车道信息.mat');
Vposition = Varin2.Vposition;  %调用了之前的数据S
end
    
% 
% Varin=load ('车辆位置信息.mat');%这两行都要一起用，其中CARposition是默认的不能改动
% Vposition = Varin.CARposition;  %调用了之前的数据S
% Vposition = Varin.Vposition;  %调用了之前的数据S
