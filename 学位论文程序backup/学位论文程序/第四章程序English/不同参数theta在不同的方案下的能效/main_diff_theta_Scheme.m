...
clear
% 总初始化
T=100; M=6;H=100; det=1;roadwidth=30;roadlength=3000;VUAV_max=45;W=10^(3);Delta=1e-6;%背景噪声是%共有70个时隙 %solt length   TTT=T*det;%time span
fix_x = 1000;  % x坐标  
fix_y = 15; % y坐标  
...
theta=0.1;%乘在E_VR之前，无人机的会比较大，这是一个系数
...
Total_sum(programmatic,thetatheta)=0.01*sum(Total_T(:));
...
% Varin=load ('data.mat');%这两行都要一起用，其中CARposition是默认的不能改动
% data = Varin.data;  %调用了之前的数据S
b = bar(data');
grid on
...
legend('\fontname{Times New Roman}OPTS','\fontname{Times New Roman}FTS','\fontname{Times New Roman}FPHS')
...
%   indices =[57 62 67 72 77];
                indices =[62 68 72 78 82];
