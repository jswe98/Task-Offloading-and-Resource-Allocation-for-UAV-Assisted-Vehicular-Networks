...
% P(1,:)=[-6.06281251985303,-6.08617489058582,-6.17506811644933,-6.21467388468052,-6.30088187750524];
Lamda(1,:)=linspace(20,20,5);
Miu(1,:)=linspace(20,20,5);
for k=1:N
    fiI(N,:)=[f1,f2,f3,f4,f5];%这是每个云给边缘分配的算力
      u_iexe(k,:)=1*(t_imax*C1-(Tc+(c_exe./(f_ba*C1+fiI(N,:)))))/t_imax*(1/d_up/log(2));%这是
%     u_iexe(k,i)=1*(t_imax*C1-(Tc*C1+(c_exe./(f_ba*C1+fiI(N,:)))))./t_imax*(1/d_up/log(2));%这是执行时间的效用多乘了一个上传前缀
    for i=1:5
