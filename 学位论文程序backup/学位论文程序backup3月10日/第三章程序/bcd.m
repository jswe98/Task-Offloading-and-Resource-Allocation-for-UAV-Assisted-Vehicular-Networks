...
for bcd1=1:8
for k=1:N
    fiI(bcd1,:)=[f1,f2,f3,f4,f5];%这是每个云给边缘分配的算力
    % fiI(1,:)=ff(bcd1,:);
    u_iexe(k,:)=1*(t_imax*C1-(Tc+(c_exe./(f_ba*C1+fiI(bcd1,:)))))/t_imax*(1/d_up/log(2));%这是执行时间的效用多乘了一个上传前缀
    for i=1:5
...
for k=1:N 
    ...
    Fai(1,:)=linspace(10,10,5);
     for i=1:5
        RRR=(exp(pp(bcd1,:))*G(i,i))/(I1(i)+Delta);
        Ri=(log(C1+RRR))/log(2);
