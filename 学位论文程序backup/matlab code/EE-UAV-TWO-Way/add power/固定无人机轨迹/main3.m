...
clear
% 总初始化
T=70; M=4;H=100; det=1;roadwidth=15;roadlength=2000; %共有70个时隙 %solt length   TTT=T*det;%time span
VUAV_max=35;%无人机速度约束
W=10^(3);
for thetatheta=1:3
theta=1+0.2*thetatheta;
...
q_l_SCA=UAVposition;
for bcd=1:1 %BCD开始  可以加while形成循环
...
[q,distance2] =Qcvx(theta,E_VU,E_VR,W,Y,L_VR,rho_l,omega_l,distance_l_SCA,CARposition,VUAV_max,det,T,M,roadlength,roadwidth,UAVposition);%使用了Qcvx函数
q_l_SCA=UAVposition;%q是cvx里面的 variable   %记录本轮优化结果，供下轮使用
            for t=1:T
                for m=1:M
...
Total_theta(thetatheta,:)=sum(Total_T(:,:));
