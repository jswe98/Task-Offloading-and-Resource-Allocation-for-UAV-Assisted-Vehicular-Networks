...
for n=1:N
    for m=1:M
         ...
         C1(m,n,l)=sqrt(beta0)*array_no(m,n);
    end
end
...
for n=1:N
    for m=1:M
        for k=1:K
            C2(m,n,l)=C2(m,n,l)+sqrt(beta0)*h2(k,m,n);
        end
    end
end
%% 第二步优化功率时间
...
for n=1:N
    for m=1:M
         hz0(m,n,l)=pow_pos(abs(hz1(m,n,l)+hz2(m,n,l)),2);
    end
end
%cvx_solver SDPT3
cvx_solver SeDuMi
...
for n=1:N
    for m=1:M
        M0(m,n)=1+pm(m,n)/sigma2*(pow_p(A(m,n),2)/pow_p(a0(m,n),beta)+pow_p(B(m,n),2)/pow_p(b0(n),2)+2*max(real(C1(m,n,l))*real(C2(m,n,l))+imag(C1(m,n,l))*imag(C2(m,n,l)),0)/(pow_p(a0(m,n),beta/2)*b0(n)));
    end
end
%定义M1
for n=1:N
    for m=1:M
        M1(m,n)=-pm(m,n)/sigma2*(beta*pow_p(A(m,n),2)/pow_p(a0(m,n),beta+1)+beta*max(real(C1(m,n,l))*real(C2(m,n,l))+imag(C1(m,n,l))*imag(C2(m,n,l)),0)/(pow_p(a0(m,n),beta/2+1)*b0(n)));
    end
end
%定义M2
for n=1:N
    for m=1:M
        M2(m,n)=-pm(m,n)/sigma2*(2*pow_p(B(m,n),2)/pow_p(b0(n),3)+2*max(real(C1(m,n,l))*real(C2(m,n,l))+imag(C1(m,n,l))*imag(C2(m,n,l)),0)/(pow_p(a0(m,n),beta/2)*pow_p(b0(n),2)));
    end
end
%cvx_clear
cvx_solver SDPT3
