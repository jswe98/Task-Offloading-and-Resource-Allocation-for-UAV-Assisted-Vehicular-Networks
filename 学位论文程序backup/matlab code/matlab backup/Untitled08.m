...
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
    ...
    for i=1:5
        test1=(varsigma+1/(Dmax-(fc+c_exe/(f_ba+fi(k,i)))))*(Ith+Delta)/tau;%这是固定f后的常数项
...
for u=1:N
   ...
   f5= fi(u+1,5);
    for i=1:5
        test2=(varsigma+1/(Dmax-(fc+c_exe/(f_ba+fi(u,i)))))*(Ith+Delta)/tau;%这是固定f后的常数项
        Ksai(u+1,:)=Ksai(u,:)+100/u*(test2+exp(P(k,i))*(log(1-e2)-G(j,j)));
    end    
