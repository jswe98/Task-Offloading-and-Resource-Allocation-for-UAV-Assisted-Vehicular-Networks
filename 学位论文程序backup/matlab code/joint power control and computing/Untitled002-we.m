...
h=[1.24441710591795,1.27173758112269,1.03809604488805,1.27401275684171,1.18970777386762;1.02926212149982,1.08354946566011,1.16406445576150,1.28725205063029,1.28946656055978;1.04728392450326,1.29117783452818,1.28715008447288,1.14561269461685,1.24008414066664;1.04256590158816,1.12652838478788,1.27472065755672,1.23766219886787,1.28784772791787;1.19672220974698,1.01071350357226,1.25473879176063,1.28019797432727,1.20362054645733];
Delta=1e-6;%背景噪声
...
P(1,:)=[-8,-8,-8,-8,-8]; 
Lamda(1,:)=linspace(180,180,5);
Miu(1,:)=linspace(20,20,5);
fi(1,:)=linspace(1,1,5);%这是每个云给边缘分配的算力
for k=1:N
    for i=1:5
    I1(i)=exp(P(k,:))*G(:,i)+Delta-G(i,i)*exp(P(k,i));
    SINR(k,i)=G(i,i)*exp(P(k,i))/I1(i);      
    arfa(k,i)=SINR(k,i)/(1+SINR(k,i));
    ZZ=0.3+(t_imax-(Tc+bate1*c_exe/f_ba+bate2*c_exe./fi(1,:)))/t_imax;%这个是前面的系数Z
...
%     end
    for i=1:5
        Miu(k+1,i)=Miu(k,i)-exp(P(k,:))*G(:,i)+(exp(log(2)*(1+tau*varsigma)/(tau*(Dmax-c_exe./fi(k,i))))-1)*(Ith+Delta)/log(1/(1-e2));%%这里需要换乘求导后的结果
    end  
