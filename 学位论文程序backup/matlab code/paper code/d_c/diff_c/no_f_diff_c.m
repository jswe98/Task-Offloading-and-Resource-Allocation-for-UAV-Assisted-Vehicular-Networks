function [no_f] = no_f_diff_c(nub_a,nub_b)
clc
close all
...
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
    ...
    for i=1:5
        ...
        Miu(k+1,i)=max(0,Miu(k,i)+1000/k*(test1+exp(P(k,i))*(log(1-e2)-G(j,j))));%%这里需要换乘求导后的结果
    end  
end
    for i=1:5
...
%  no_f= [D(1,1), D(2,1), D(3,1), D(4,1), D(5,1)];
% no_f = [D(1,1), D(2,1), D(3,1), D(4,1)];
no_f= [D(1,1), D(2,1), D(3,1)];
%{
plot(exp(P(:,1)),'-+r');
hold on
plot(exp(P(:,2)),'-og');
