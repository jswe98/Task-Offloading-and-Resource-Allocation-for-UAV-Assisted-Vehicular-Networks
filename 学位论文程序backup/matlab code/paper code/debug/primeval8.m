...
clear
Z=6;
V1=linspace(30,30,Z);%发送车的速度
% V1=[30,30,30,30,30];%发送车的速度
V2=linspace(0,0,Z);%接收基站的速度
nub_a=1.5;
nub_b=0.1;
CE=linspace(0,0,Z);
v=zeros(Z,Z);%车与基站之间的相对速度矩阵
h=zeros(Z,Z);%车与基站之间的先前时刻的值
for i=1:Z
    for j=1:Z
        v(i,j)=abs(V1(i)-V2(j));
        h(i,j)=1+0.3*rand(1);
...
arfa=zeros(N,Z);
SINR=zeros(N,Z);
Pmax=log(0.5)*ones(N,Z);
C1=linspace(1,1,Z);
S=zeros(N,Z);
s=zeros(N,Z);
Ith=2e-6;%干扰阈值Gama要给相应的存储空间
Temp=zeros(N,Z);%每次迭代收敛的最优功率暂存区 40应为Z
% Z=1;%迭代次数每次加一
I1=zeros(N,Z);
Total_T=zeros(N,Z);
P=zeros(N,Z);
...
u_iexe=zeros(N,Z);
P(1,:)=linspace(-9,-9,Z);
Lamda(1,:)=linspace(20,20,Z);
Miu(1,:)=linspace(20,20,Z);
fftemp=linspace(6,6,Z);
for k=1:N
...
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
    ...
    Ksai(1,:)=linspace(3000,3000,Z);
    Fai(1,:)=linspace(10,10,Z);
     for i=1:Z
