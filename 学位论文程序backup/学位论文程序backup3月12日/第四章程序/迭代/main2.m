...
(theta*sum(E_VR_real)+(1-theta)*sum(E_VU_real));
             if (abs(EE_total_recod(BCDl+1)-EE_total_recod(BCDl))/EE_total_recod(BCDl+1)>0.001) %可以加入BCD何时结束，判定方法为abs()>0.1
                 flagBCD=0;
                 BCDl=BCDl+1; 
             end
bound1=1;
...
[Pbound,qbound] = deal(zeros(bound2-bound1+1,M));
   ...
   Total_theta(BCDl,:)=0.01*sum(Total_T,1);          
      end
Total_thetaSUM=sum(0.01* Total_theta,2)';  
...
grid on;
box on;
plot( Total_thetaSUM(1, :), '-xb', 'LineWidth', 1.5);
% plot(0.001* EE_total_recod(1, :), '-sr', 'LineWidth', 1.5);
% 图形标题和标签
%title('能效');
xlabel('迭代次数');
ylabel('能效 (bits/J)');
