...
for n=1:N
    for m=1:M
        M0(m,n)=1+abs(pm(m,n)/sigma2*(pow_p(A(m,n),2)/pow_p(a0(m,n),beta)+pow_p(B(m,n),2)/pow_p(b0(n),2)+2*(real(C1(m,n,l))*real(C2(m,n,l))+imag(C1(m,n,l))*imag(C2(m,n,l)))/(pow_p(a0(m,n),beta/2)*b0(n))));
    end
end
%定义M1
for n=1:N
    for m=1:M
        M1(m,n)=-abs(pm(m,n)/sigma2*(beta*pow_p(A(m,n),2)/pow_p(a0(m,n),beta+1)+beta*(real(C1(m,n,l))*real(C2(m,n,l))+imag(C1(m,n,l))*imag(C2(m,n,l)))/(pow_p(a0(m,n),beta/2+1)*b0(n))));
    end
end
%定义M2
for n=1:N
    for m=1:M
        M2(m,n)=-abs(pm(m,n)/sigma2*(2*pow_p(B(m,n),2)/pow_p(b0(n),3)+2*(real(C1(m,n,l))*real(C2(m,n,l))+imag(C1(m,n,l))*imag(C2(m,n,l)))/(pow_p(a0(m,n),beta/2)*pow_p(b0(n),2))));
    end
end
%cvx_clear
cvx_solver SDPT3
