...
for bcd=1:90
for k=1:N
    fik(k,:)=[f1,f2,f3,f4,f5];%这是每个云给边缘分配的算力
    u_iexe(k,:)=(t_imax*C1-(Tc+(c_exe./(f_ba*C1+fik(k,:)))))/t_imax*(1/d_up/log(2));%这是执行时间的效用多乘了一个上传前缀
    for i=1:5
...
for u=1:N
   ...
   f5= fi(u+1,5);
    for i=1:5       
        Ksai(u+1,i)=Ksai(u,i)+1/u*(c_exe/(fi(u,i)+f_ba)+D1(1,i)-Dmax);
    end    
%     for i=1:5       
%         sumfi(u,i)=sum(fi(u,:));
