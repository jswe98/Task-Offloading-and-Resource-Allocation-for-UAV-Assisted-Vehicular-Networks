...
%         xiajie=(tau*c_exe)./(tau*(Dmax-(fc+c_exe/(f_ba+fi(k,i))))-((log(2)*(1+tau*varsigma))./(log(C1+(log(1/(1-e2))*exp(P(k,i))*G(i,i))))./(exp(P(k,:))*I(:,i))));
        fi(u+1,:)=max(0,fi(u,:)+sqrt(BR/(((Ith+Delta)+(AA+varsigma)*Dmax)*(Ksai(u,i))))-f_ba);
    end    
   f1= fi(u+1,1);
   f2= fi(u+1,2);
   f3= fi(u+1,3);
   ...
   f5= fi(u+1,5);
    for i=1:5  
        ...
        Ksai(u+1,i)=Ksai(u,i)+1/u*((Ith+Delta)*fi(u,i)+(varsigma+AA)*(fi(u,i)*Dmax-c_exe));
    end    
end    
Z=Z+1;
end