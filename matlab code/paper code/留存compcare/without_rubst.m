    ...
    for i=1:5
        for j=1:5
            if j~=i
               ...
               S(k,j)=-Miu(k,j)*(c_exe/(fi(k,i)+f_ba)+D1(1,i)-Dmax)-G(j,j);
            end
        end
               s(k,i)=sum(S(k,:));
    end
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
    ...
    for i=1:5
        ...
        Miu(k+1,i)=max(0,Miu(k,i)+100/k*(c_exe/(fi(k,i)+f_ba)+D1(1,i)-Dmax));%%这里需要换乘求导后的结果
    end  
end
for u=1:N
    fi(1,:)=[f1,f2,f3,f4,f5];
   ...
   sumfi(k,1)=sum(fi(k,:));       
    for i=1:5       
        Ksai(k+1,i)=Ksai(k,i)+1000/k*(c_exe/(fi(k,i)+f_ba)+D1(1,i)-Dmax);
    end    
    for i=1:5       
        sumfi(k,i)=sum(fi(k,:));
        Fai(k+1,i)=max(0,Fai(k,i)+1/u*(sumfi(k,i)-f_total));
    end  
end    
Z=Z+1;
end
