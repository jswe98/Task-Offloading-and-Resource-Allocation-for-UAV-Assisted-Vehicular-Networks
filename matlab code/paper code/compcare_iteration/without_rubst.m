function [without_rubst] = without_rubst(~)
clc
close all
clear
...
%    P(k,:)=linspace(-9,-9,5);
    ...
    for i=1:5   
    I1(i)=exp(P(k,:))*G(:,i)+Delta-G(i,i)*exp(P(k,i));
    SINRt(k,i)=G(i,i)*exp(P(k,i))/I1(i);      
    arfa(k,i)=SINR(k,i)/(1+SINR(k,i));
    Total_T(u)=dot(log(C1+SINR(k,:)),u_iexe(u,:)); %这是第一个子问题的和的形式
    end
...
utemp=u_iexe;
    for i=1:N  
 Total_Temp(i,:)=u_iexe(i,1)*log(1+SINR(i,1))+u_iexe(i,2)*log(1+SINR(i,2))+u_iexe(i,3)*log(1+SINR(i,3))+u_iexe(i,4)*log(1+SINR(i,4))+u_iexe(i,5)*log(1+SINR(i,5))    ;
    end 
without_rubst=Total_Temp(:,:);
