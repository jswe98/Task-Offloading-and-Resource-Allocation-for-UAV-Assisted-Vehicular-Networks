function [JEE] = JEEdiff_c(nub_a,nub_b)
clc
close all
...
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
   ...
   sumfi(k,1)=sum(fi(k,:));     
    ...
    for i=1:5       
        Ksai(k+1,i)=Ksai(k,i)+0.1/k*(Dmax-c_exe/(fi(k,i)+f_ba)+D1(1,i));
    end    
...
JEE = [D(1,1), D(2,1), D(3,1)];
% JEE= [D(1,1), D(2,1), D(3,1), D(4,1), D(5,1)];
% JEE = [D(1,1), D(2,1), D(3,1), D(4,1)];
% JEE= [D(1,1), D(2,1), D(3,1)];
% b = bar(data);
% ch = get(b,'children');
...
% cc2=1000*aqa(2);
% cc3=1000*aqa(3);
% set(gca,'XTickLabel',{'c=cc1','c=cc2','c=cc3'})
% ylabel('EE');
