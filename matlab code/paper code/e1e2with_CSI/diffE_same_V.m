function [diffE_same_V] = diffE_same_V(~)
clc
close all
clear
...
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
    ...
    for i=1:5
         Lamda(k+1,i)=max(0,Lamda(k,i)+10000000/k*(Delta+I(i)));
    end 
...
Total_T(k)=dot(log(C1+SINR(k,:)),u_iexe(k,:));
SE(EE)=Total_T(N,1);
Temp(EE,:)=exp(P(N,:));
  end
diffE_same_V=SE;
% data = [SE(1); SE(2);SE(3);SE(4);SE(5)] ;
% b = bar(data');
% ch = get(b,'children');
...
%  ge=[12.4909907272466,12.4981265358249,12.5030022249711,12.5070540624778,12.5101524256155]
% figure
% plot(SE(1,:),'-+r','linewidth',2);
% hold on
% plot(ge(1,:),'-+b','linewidth',2);
