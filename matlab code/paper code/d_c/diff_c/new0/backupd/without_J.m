...
for k=1:N
    ...
    u_iexe(k,:)=0.5*(t_imax*C1-(Tc+(c_exe./(f_ba*C1+fiI(N,:)))))/t_imax*(1/d_up/log(2));%这是执行时间的效用多乘了一个上传前缀
    for i=1:5
...
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
    ...
    for i=1:5
         Lamda(k+1,i)=max(0,Lamda(k,i)+1/k*(exp(P(k,:))*G1(:,i)+Delta));
    end 
    if  max(abs(Lamda(k+1,:)-Lamda(k,:)))<=2.4e-4   %该数值可改，表示乘子在一定的程度下就认为已经稳定了
        Lamda(k+1,:)=Lamda(k,:);
    end
    for i=1:5
...
for u=1:N
   ...
   sumfi(u,1)=sum(fi(u,:));     
    ...
    for i=1:5       
        Ksai(u+1,i)=Ksai(u,i)+0.1/u*(Dmax-c_exe/(fi(u,i)+f_ba)+D1(1,i));
    end    
