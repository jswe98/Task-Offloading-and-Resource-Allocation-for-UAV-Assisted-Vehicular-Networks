...
%     P(k+1,:)=min(log(u_iexe(k,:).*arfa(k,:)/log(2))-log(s(k,:)),Pmax(k,:));
    ...
    for i=1:5
        test1=(varsigma+1/(Dmax-c_exe/fi(k,i)))*(Ith+Delta);%z这是固定f后的常数项
        Miu(k+1,i)=max(0,Miu(k,i)+10/k*(test1+exp(P(k,i))*tau*(log(1-e2)-G(j,j))));%%这里需要换乘求导后的结果
    end  
end
for u=1:N
    fi(1,:)=[f1,f2,f3,f4,f5];
    Ksai(1,:)=[2,2,2,2,2];
    Ksai(1,:)=linspace(10,10,5);
    Fai(1,:)=linspace(20,20,5);
    for i=1:5
        Ri=(log(C1+SINR(k,:)))/log(2);
        BR=Ri.*c_exe/(t_imax*d_up); 
        AA=Dmax*(tau*exp(P(k,i))*(log(1-e2)-G(i,i)))/(Ith+Delta);
        BB=(1+AA+varsigma*Dmax)*Ksai(u,i);
        fi(u+1,:)=max(0,fi(u,:)+1000000/u*sqrt(BR/BB)-f_ba);
    end    
   f1= fi(u+1,1);
   f2= fi(u+1,2);
   f3= fi(u+1,3);
   ...
   f5= fi(u+1,5);
    for i=1:5  
        CC=(1+AA+varsigma*Dmax)*fi(u+1,i)-c_exe*(varsigma+AA/Dmax);
        Ksai(u+1,i)=Ksai(u,i)+100000/u*CC;
    end    
end    
Z=Z+1;
end