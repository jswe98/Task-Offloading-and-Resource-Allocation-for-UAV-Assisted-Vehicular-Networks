...
clear
% 总初始化
T=90; M=6;H=100; det=1;roadwidth=15;roadlength=2000; %共有70个时隙 %solt length   TTT=T*det;%time span
VUAV_max=45;%无人机速度约束
W=10^(3);
theta=0.4;%乘在E_VR之前，无人机的会比较大，这是一个系数
for thetatheta=1:3
theta=0.2+0.2*thetatheta;
Delta=1e-6;%背景噪声是
Pm=0.3*ones(T,M);
R=zeros(T,M);
...
%for bcd=1:5 %BCD开始  可以加while形成循环
...
hold off;
xlabel('马路长度', 'FontSize', 26);
ylabel('马路宽度', 'FontSize', 26);
xlim = get(gca, 'XLim');
ylim = get(gca, 'YLim');
y_center = mean(ylim);
% 绘制无人机轨迹优化
...
legend(legend_entries, 'Location', 'best', 'FontSize', 20);
set(gca, 'XTickLabel', get(gca, 'XTickLabel'), 'YTickLabel', get(gca, 'YTickLabel'), 'FontSize', 26);  % 设置刻度字体大小为 14  
YY=1-X';
...
% plot(data1,'-*b'); %这个是一下子全都画出来
hold on;
plot(Total_theta(1,indices),'-xb', 'LineWidth', 1.5);   
