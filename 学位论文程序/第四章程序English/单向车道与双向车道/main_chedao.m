...
clear
% 总初始化
T=70; M=6;H=100; det=1;roadwidth=30;roadlength=3000;VUAV_max=45;W=10^(3);Delta=1e-6;%背景噪声是%共有70个时隙 %solt length   TTT=T*det;%time span
fix_x = 2500;  % x坐标  
fix_y = 30; % y坐标  
%cvx_quiet(true);
...
% for programmatic = 1:2  ;thetatheta=1; 
chedaoindices = [2, 1]; 
for chedao=chedaoindices
...
[GVR,GVU,distanceVR,distanceVU,CARposition,L]=CARinfo(H,T,M,det,roadlength,roadwidth,UAVcvx,programmatic,thetatheta,BCDl,chedao);%使用了汽车信息函数
...
figure;
plot(zhishu,Total_chedao(1,zhishu), '-*b', 'LineWidth', 1)
hold on;
grid on; 
box on;
plot(zhishu,Total_chedao(2,zhishu), '-dr', 'LineWidth', 1)
xlabel('\fontname{Times New Roman}T(s)');
ylabel('\fontname{Times New Roman}EE \fontname{Times New Roman}(bits/J)');
legend('\fontname{Times New Roman}One-way street','\fontname{Times New Roman}Two-way street', 'Location', 'best');
