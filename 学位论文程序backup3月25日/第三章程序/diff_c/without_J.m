function [without_J] = without_J(CC,CE,ddd)
% clc
% close all
% clear
...
%    P(k,:)=linspace(-9,-9,5);
      ...
      u_iexe(u,:)=1*(t_imax*C1-(Tc+(c_exe./(f_ba*C1+fiI(N,:)))))/t_imax*(1/d_up/log(2));%这是
%     u_iexe(u,:)=1*(t_imax*C1-(Tc+(c_exe./(f_ba*C1+fiI(N,:)))))/t_imax*(1/d_up/log(2));%这是执行时间的效用多乘了一个上传前缀
    for i=1:5   
    I1(i)=exp(P(1,:))*G(:,i)+Delta-G(i,i)*exp(P(1,i));
    SINRt(1,i)=G(i,i)*exp(P(1,i))/I1(i);      
   ...
   sumfi(u,1)=sum(fi(u,:));     
    for i=1:5       
        sumfi(u,i)=sum(fi(u,:));
        Fai(u+1,i)=max(0,Fai(u,i)+1.5/u*(sumfi(u,i)-f_total));
    end  
    if  max(abs(Fai(u+1,:)-Fai(u,:)))<=1e-6  %该数值可改，表示乘子在一定的程度下就认为已经稳定了
        Fai(u+1,:)=Fai(u,:);
    end
    for i=1:5       
        Ksai(u+1,i)=Ksai(u,i)+0.1/u*(Dmax-c_exe(i)/(fi(u,i)+f_ba)+D1(1,i));
    end    
    if  max(abs(Ksai(u+1,:)-Ksai(u,:)))<=1e-6  %该数值可改，表示乘子在一定的程度下就认为已经稳定了
        Ksai(u+1,:)=Ksai(u,:);
    end
% fff=fi(u+1,:)
end
for k=1:N
    fiI(N,:)=[f1,f2,f3,f4,f5];%这是每个云给边缘分配的算力
    u_iexe(k,:)=(t_imax*C1-(Tc+(c_exe./(f_ba*C1+fiI(N,:)))))/t_imax*(1/d_up/log(2));%这是执行时间的效用多乘了一个上传前缀
    for i=1:5
...
utemp=u_iexe;
    for i=1:N  
 Total_Temp(i,:)=u_iexe(i,1)*log(1+SINR(1,1))+u_iexe(1,2)*log(1+SINR(1,2))+u_iexe(1,3)*log(1+SINR(1,3))+u_iexe(i,4)*log(1+SINR(i,4))+u_iexe(i,5)*log(1+SINR(i,5))    ;
    end 
without_JJ=Total_Temp(:,:);
D(CC)=without_JJ(N,1);
without_J=D(CC);